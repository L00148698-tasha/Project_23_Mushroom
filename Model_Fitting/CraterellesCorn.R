...
target_species <- "Craterellus cornucopioides"

# Create a new data frame with only the rows for the target species
craterellus_cor <- alldata[alldata$species == target_species, ]

#split the data

present <- craterellus_cor[craterellus_cor$present == 1, ]
absent<-craterellus_cor[craterellus_cor$present == 0, ]
...
writeRaster(pr,"predictCraterellesCorn.tif",overwrite=T)
