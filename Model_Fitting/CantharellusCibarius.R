...
target_species <- "Cantharellus cibarius"

# Create a new data frame with only the rows for the target species
cantharellus_cib <- alldata[alldata$species == target_species, ]

#split the data

present <- cantharellus_cib[cantharellus_cib$present == 1, ]
absent<-cantharellus_cib[cantharellus_cib$present == 0, ]
...
writeRaster(pr,"predictCantharellusCib.tif",overwrite=T)
