...
target_species <- "Craterellus tubaeformis"

# Create a new data frame with only the rows for the target species
craterellus_tub <- alldata[alldata$species == target_species, ]

#split the data

present <- craterellus_tub[craterellus_tub$present == 1, ]
absent<-craterellus_tub[craterellus_tub$present == 0, ]
...
writeRaster(pr,"predictCraterellusTub.tif",overwrite=T)
