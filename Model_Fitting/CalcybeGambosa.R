...
target_species <- "Calocybe gambosa"

# Create a new data frame with only the rows for the target species
calocybe_gam <- alldata[alldata$species == target_species, ]

#split the data

present <- calocybe_gam[calocybe_gam$present == 1, ]
absent<-calocybe_gam[calocybe_gam$present == 0, ]
...
writeRaster(pr,"predictCalocybeGam.tif",overwrite=T)
