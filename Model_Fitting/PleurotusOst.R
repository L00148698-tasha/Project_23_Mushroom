...
target_species <- "Pleurotus ostreatus"

# Create a new data frame with only the rows for the target species
pleurotus_ost <- alldata[alldata$species == target_species, ]

#split the data

present <- pleurotus_ost[pleurotus_ost$present == 1, ]
absent<-pleurotus_ost[pleurotus_ost$present == 0, ]
...
writeRaster(pr,"predictPleurotusOst.tif",overwrite=T)
